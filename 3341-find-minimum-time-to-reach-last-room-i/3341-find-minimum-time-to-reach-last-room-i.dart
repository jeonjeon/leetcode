class Solution {
  int minTimeToReach(List<List<int>> mt) {
    ...
    final dp = List.generate(mt.length, (_) => List.filled(mt[0].length, 1000000300));
    dp[0][0] = 0;
    final pq = PriorityQueue<(int, int, int)>((a, b) => a.$3.compareTo(b.$3));
    pq.add((0, 0, 0));
    while (pq.isNotEmpty){
      final (y, x, time) = pq.remove();
      if (time > dp[y][x]) continue;
      for (final (dy, dx) in dir){
        final ny = y + dy;
        final nx = x + dx;
        if (ny < 0 || ny >= mt.length) continue;
        if (nx < 0 || nx >= mt[0].length) continue;
        final nTime = max(mt[ny][nx] + 1, time + 1);
